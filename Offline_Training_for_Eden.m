...
duration = 3; % Sample data only consist of 4s-long trials, Change to 5 for your own data
AllData = zeros(4, fs, 0); % (chan x time x trials) , Change channel number to 2 for your own data
...
AllLabel = repelem(Stimulus_Type, 3); % For each trial, 3 segments of 1s data is extracted, Change to 5 for your own data
% Save data
...
TrainData = X;
TrainLabel = label;
SVMModel = fitcecoc(TrainData, TrainLabel, 'Learners', templateSVM('KernelFunction', 'polynomial', 'PolynomialOrder', 2, 'KernelScale', 'auto'), 'Coding', 'onevsall', 'ClassNames', [0 1 2 3]);
