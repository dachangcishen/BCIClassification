...
% create a length * 1 empty cell
length = size(AllData,3);
AllDataC = cell(length, 1);

% traverse all the data
for i = 1:length
    % get a 2x256 slice in the 3D data
    slice = AllData(:, :, i);    
...
numObservations = numel(AllDataC);
[idxTrain,idxTest] = trainingPartitions(numObservations, [0.9 0.1]);

XTrain = AllDataC(idxTrain);
TTrain = AllLabel(idxTrain);
XTest = AllDataC(idxTest);
TTest = AllLabel(idxTest);
